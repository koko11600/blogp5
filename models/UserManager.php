...
class UserManager extends Model
{
    ...
    {
        /* Ici modification password check mdp sur les deux ( correspond ) sinon erreur */
        $this->getBdd();
        $newFields = array_map('htmlspecialchars', $_POST);
        $result = "Votre mot de passe à bien etait modifier";

        if ($newFields['password'] != $newFields['confirmPassword']) {
            $errors = "Vos mots de passe sont différents";
        }
        if (empty($errors)) {
            $req = self::$bdd->prepare("UPDATE $table SET password = ?, dateUpdated = ?  WHERE id = ?");
            $req->execute(array(md5($newFields['password']), date("Y-m-d H:i:s"), $_SESSION['id']));
            $req->closeCursor();
            return $result;
        } else {
            return $errors;
    ...
    private function modifyUserById($table, $id)
    {
        ...
        $req = self::$bdd->prepare("UPDATE $table SET role = 1 WHERE id = ? ");
        $req->execute(array($id));
        $user = $req->fetch(PDO::FETCH_ASSOC);
        $req->closeCursor();
